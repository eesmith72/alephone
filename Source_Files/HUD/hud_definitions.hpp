...
#define GET_CURRENT_INVENTORY_SCREEN(p) ((p)->hud_flags & INVENTORY_MASK_BITS)

//#define INVENTORY_IS_DIRTY(p) ((p)->hud_flags & INVENTORY_DIRTY_BIT)
#define SET_INVENTORY_DIRTY_STATE(p) ((p)->hud_flags |= (uint16_t)INVENTORY_DIRTY_BIT)

//#define INTERFACE_IS_DIRTY(p) ((p)->hud_flags & INTERFACE_DIRTY_BIT)
//#define SET_INTERFACE_DIRTY_STATE(p, v) ((v)?((p)->hud_flags |= INTERFACE_DIRTY_BIT):(p)->hud_flags &= ~INTERFACE_DIRTY_BIT)
