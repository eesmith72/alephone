/*
LUA_HUD_OBJECTS.CPP -- Implements Lua HUD objects and globals
...
#include "network_games.h" // get_network_compass_state
...
static float Lua_HUDColor_Lookup(lua_State *L, int pos, int idx, const char *name1, const char *name2) // TODO: wtf
{
	float ret = 1.0f;
...
static SDL_Color Lua_Get_HUDColor(lua_State *L, int pos)
{
    float r = Lua_HUDColor_Get_R(L, pos); // these are 0.0-1.0 for OGL, but we want SDL
    float g = Lua_HUDColor_Get_G(L, pos);
    float b = Lua_HUDColor_Get_B(L, pos);
    float a = Lua_HUDColor_Get_A(L, pos);
    return to_sdl_color(r, g, b, a);
}
...
{
//	lua_pushnumber(L, Lua_Image_Crop_Rect::Object(L, 1)->crop_rect.x); // TODO: FIX: Lua needs to wrap Canvas* in its own class
	return 1;
}
...
{
	//Lua_Image_Crop_Rect::Object(L, 1)->crop_rect.x = lua_tonumber(L, 2); // TODO: FIX: again, it's for Lua to wrap
  return 0;
}
...
{
    Lua_HUDInstance()->canvas->draw_image(Lua_Image::Object(L, 1), {(int32_t)lua_tonumber(L, 2), (int32_t)lua_tonumber(L, 3)});
	return 0;
}
...
int Lua_Images_New(lua_State *L)
{
    ...
    if (!lua_isnil(L, -1))
    {
        int32_t resource_id = (int32_t)lua_tointeger(L, -1);
        
        SDL_Surface* surface = get_pict_resource_from_images(resource_id);
        if (!surface) // resource id not found
        {
            lua_pushnil(L);
            return 1;
        }
    ...
    SDL_Surface* surface = image.copy_to_surface();
    if (!surface)
    {
        lua_pushnil(L);
        return 1;
...
{
    Lua_HUDInstance()->canvas->draw_shape(Lua_Shape::Object(L, 1), {(int32_t)lua_tonumber(L, 2), (int32_t)lua_tonumber(L, 3)});
	return 0;
}
...
class Lua_Font : public L_ObjectClass<Lua_Font_Name, font_t*>
{
public:
	float m_font_scale;
    	
	static Lua_Font *Push(lua_State *L, font_t *fs);
	static float Scale(lua_State *L, int index);
    static void SetScale(lua_State *L, int index, float new_scale);
};

Lua_Font *Lua_Font::Push(lua_State *L, font_t *fs)
{
...
int Lua_Font_Draw_Text(lua_State *L)
{
	...
	int32_t x = lua_tonumber(L, 3); // TODO: not sure what these numbers are? presumably OGL coords
	int32_t y = lua_tonumber(L, 4);
    SDL_Color color = Lua_Get_HUDColor(L, 5);

    // TODO: FIX scaling
    // Lua_HUDInstance()->draw_text(Lua_Font::Object(L, 1), str, x, y, r, g, b, a, Lua_Font::Scale(L, 1));
    
    Lua_HUDInstance()->canvas->draw_text(str, Lua_Font::Object(L, 1), color, {x, y});
	return 0;
}

static int Lua_Font_Get_Size(lua_State *L)
{
    lua_pushnumber(L, Lua_Font::Object(L, 1)->key.size * Lua_Font::Scale(L, 1));
	return 1;
}

static int Lua_Font_Get_Style(lua_State *L)
{
    lua_pushnumber(L, Lua_Font::Object(L, 1)->key.style);
	return 1;
}
...
{
	//lua_pushnil(L);
    lua_pushnumber(L, Lua_Font::Object(L, 1)->key.font_id);
	return 1;
}

static int Lua_Font_Get_Line_Height(lua_State *L)
{
    lua_pushnumber(L, Lua_Font::Object(L, 1)->line_height * Lua_Font::Scale(L, 1));
	return 1;
}
...
{
    /*
    Font f = {"Monaco", 12, styleNormal, 0, "mono"};
...
static int Lua_HUDPlayer_Weapon_Name_Rect_Get_X(lua_State *L)
{
    ...
    if (startx == NONE)
    {
        SDL_Rect r = get_hud_rect(_weapon_display_rect);
        startx = r.x;
    }
...
static int Lua_HUDPlayer_Weapon_Name_Rect_Get_Width(lua_State *L)
{
    ...
    {
        SDL_Rect r = get_hud_rect(_weapon_display_rect);
        if (startx == NONE) startx = r.x;
        if (endx == NONE) endx = r.x + r.w;
    }
...
{
	blip_info_t info = get_motion_sensor_blip(Lua_MotionSensor_Blip::Index(L, 1));
	Lua_SensorBlipType::Push(L, (int32_t)info.mtype);
	return 1;
}
...
{
	return index >= 0 && index < get_motion_sensor_blip_count();
}
...
{	
	lua_pushnumber(L, get_motion_sensor_blip_count());
	return 1;
}
...
{
	lua_pushboolean(L, !(GET_GAME_OPTIONS() & _motion_sensor_does_not_work) && get_motion_sensor_active());
	return 1;
}
...
{
	lua_pushboolean(L, zoom_is_active());
    return 1;
}
...
{
    SDL_Rect r = get_interface_rect(Lua_InterfaceRect::Index(L, 1));
...
int Lua_HUDObjects_register(lua_State *L)
{
	...
	Lua_SensorBlipType::Valid = Lua_SensorBlipType::ValidRange(NUMBER_OF_BLIP_TYPES);
	
	Lua_SensorBlipTypes::Register(L);
	Lua_SensorBlipTypes::Length = Lua_SensorBlipTypes::ConstantLength(NUMBER_OF_BLIP_TYPES);
