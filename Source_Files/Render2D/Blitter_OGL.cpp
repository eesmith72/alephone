...
// which Blitter_OGL::render_to_screen will tile back together when rendering to screen.
void Blitter_OGL::create_texture_tiles()
{
    ...
    // but not going to support that right now as it requires Blitter to know in advance how an image will be used.)
...
void Blitter_OGL::render_to_screen(const SDL_Rect* dst_rect, const SDL_Rect* src_rect)
{
    if (!m_surface)
    {
        log_error("Called Blitter_OGL::render_to_screen but a Surface wasn't loaded. This is probably a bug.");
        return;
    }
    if (m_tiles.empty())
    {
