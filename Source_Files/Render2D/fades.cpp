...
// LG: now specifies whether music should be cut or not // EES: whyyyyyy..... (it's really a badly named function)
...
void animate_ui_fade_out_blocking(bool fade_music)
{
    ...
    if (fade_music) // start fading music
    {
...
{
    if (index < 0 && index >= NUMBER_OF_FADE_TYPES) throw_ao_exception("Out of bounds: %d", 1, index);
