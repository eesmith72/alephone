...
ShapeBlitter::ShapeBlitter(short collection, short frame_index, short texture_type, short clut_index) : m_coll(BUILD_COLLECTION(collection, clut_index)), m_frame(frame_index), m_type(texture_type), m_surface(NULL), m_scaled_surface(NULL), tint_color_r(1.0), tint_color_g(1.0), tint_color_b(1.0), tint_color_a(1.0), rotation(0.0)
{
...
void ShapeBlitter::Rescale(float width, float height)
{	
...
float ShapeBlitter::Width()
{
	return m_scaled_src.w;
}

float ShapeBlitter::Height()
{
	return m_scaled_src.h;
}

int ShapeBlitter::UnscaledWidth()
{
	return m_src.w;
}

int ShapeBlitter::UnscaledHeight()
{
	return m_src.h;
}

void ShapeBlitter::OGL_Draw(const Image_Rect& dst)
{
	// Set up texture
	TextureManager TMgr;
	...
	// Draw shape
	if (ogl_preferences.Use_sRGB && TMgr.TextureType != OGL_Txtr_WeaponsInHand && TMgr.TextureType != OGL_Txtr_HUD) // also not hud just in case
	{
...
void ShapeBlitter::SDL_Draw(SDL_Surface *dst_surface, const Image_Rect& dst) // only place this is used is in Canvas_SDL::draw_shape, where it's currently commented out as the only code which _might_ use it is a Lua HUD plugin; see: Lua_Shape_Draw
{
        ...
        m_scaled_surface->h != m_scaled_src.h)
    {
        ...
        if (m_scaled_src.w != m_src.w || m_scaled_src.h != m_src.h)
            m_scaled_surface = SDL_Resize(m_surface, m_scaled_src.w, m_scaled_src.h, false, 4);
        else
