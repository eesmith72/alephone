...
widget::widget() : widget_theme_id(-1), active(false), dirty(false), enabled(true),
...
widget::widget(int32_t widget_theme_id) : widget_theme_id(widget_theme_id), active(false), dirty(false), enabled(true), 
                                       identifier(NONE), owning_dialog(nullptr), saved_min_width(0), saved_min_height(0), label_widget(0)
{
    rect.x = 0;
    rect.y = 0;
...
void w_select::click(int32_t x, int32_t y) // advance selection
{
    set_selection(selection == count() - 1 ? 0 : selection + 1);
}
...
// TODO: FIX: currently very buggy; this needs thoroughly tested to confirm everything works correctly with UTF8; see also Console.cpp
...
void w_levels::draw_item(std::vector<level_identity>::const_iterator it, Canvas* canvas, int16_t x, int16_t y, uint16_t width, bool selected)
{
    std::string str;
...
void w_file_chooser::proc()
{
    ...
    {
        ao_path path;
        ao_err err = display_read_file_dialog(path, typecode, dialog_prompt);
        if (!err)
        {
            update_filename(path);
...
void w_file_chooser::update_filename(const ao_path& path)
{
    if(std::filesystem::is_regular_file(path))
    {
        file = path;
        filename = hide_ao_filename_extension(path.filename());
