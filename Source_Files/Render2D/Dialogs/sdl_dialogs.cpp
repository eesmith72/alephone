...
// TODO: FIX: dialog Surface is fixed 640x480 and uses its own x,y offsets when drawing a dialog background in it so that dialog appears centered in screen; this centering is now a bit off due to other changes/bugs (low-priority as the Prefs dialogs still work and there's a fair chance this will be replaced anyway)
...
static SDL_Surface* tile_surface(SDL_Surface *s, int width, int height);



dialog *top_dialog = NULL;

static const SDL_Point canvas_size = {640, 480};
...
void dialog::draw_all_widgets(void)
{
    ...
    //dialog_canvas->draw_filled_rect({0,0,320,240}, {255,255,255,25}); // DEBUG (shows the Surface size and center in gray)
    //dialog_canvas->draw_filled_rect({320,240,320,240}, {255,255,255,25}); // DEBUG
...
void dialog::process_event(SDL_Event &e)
{
  ...
  if (!handled) {
	  ...
	  if (e.type == SDL_MOUSEMOTION)
	  {
		  ...
		  {	  
			  int num = find_widget(position.x, position.y);
			  if (num >= 0)
			  {
		  ...
		  {
			  target->event(e);
			  target->mouse_move(position.x - rect.x - target->rect.x, position.y - rect.y - target->rect.y);
		  }
	  }
	  else if (e.type == SDL_MOUSEBUTTONDOWN)
	  {
		  ...
		  if (num >= 0)
		  {
			  ...
			  if (e.button.button == SDL_BUTTON_LEFT || e.button.button == SDL_BUTTON_RIGHT)
				  mouse_widget->mouse_down(position.x - rect.x - mouse_widget->rect.x, position.y - rect.y - mouse_widget->rect.y);
		  }
	  }
	  else if (e.type == SDL_MOUSEBUTTONUP)
	  {
		  if (mouse_widget)
		  {
			  ...
			  {
                  SDL_Point position = main_screen.get_mouse_virtual_position();
				  mouse_widget->mouse_up(position.x - rect.x - mouse_widget->rect.x, position.y - rect.y - mouse_widget->rect.y);
			  }
