...
void OverheadMap_SDL_Class::draw_polygon(short vertex_count, short *vertices, rgb_color& color)
{
	...
	// Draw polygon
	::draw_polygon_xxxx(draw_surface, vertex_array, vertex_count, pixel);
}
...
void OverheadMap_SDL_Class::draw_line(short *vertices, rgb_color &color, short pen_size)
{
	...
	// Draw line
	::draw_line_xxxx(draw_surface, v1, v2, pixel, pen_size);
}
...
void OverheadMap_SDL_Class::draw_thing(world_point2d &center, rgb_color &color, short shape, short radius)
{
	...
	switch (shape) {
		...
		case _circle_thing: {
			...
			for (int i=0; i<7; i++)
				::draw_line_xxxx(draw_surface, circle + i, circle + i + 1, pixel, 2);
			::draw_line_xxxx(draw_surface, circle + 7, circle, pixel, 2);
			break;
		}
	}
}
...
void OverheadMap_SDL_Class::draw_player(world_point2d &center, angle facing, rgb_color &color, short shrink, short front, short rear, short rear_theta)
{
	...
	// Draw triangle
	::draw_polygon_xxxx(draw_surface, triangle, 3, pixel);
}
...
{
	if (step)
		::draw_line_xxxx(draw_surface, &path_point, &location, path_pixel, 1);
	path_point = location;
}
