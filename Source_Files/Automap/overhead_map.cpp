...
void clear_automap()
{
    std::fill(AutomapLineList.begin(), AutomapLineList.end(), 0x00);
    std::fill(AutomapPolygonList.begin(), AutomapPolygonList.end(), 0x00);
}
...
void ResetOverheadMap()
{
    ...
    {
        case OverheadMap_Normal: // Default: nothing (mapping is cumulative)
            break;
        case OverheadMap_CurrentlyVisible: // No previous visibility is carried over
            clear_automap();
            break;
        case OverheadMap_All: // Everything is assumed visible
            std::fill(AutomapLineList.begin(), AutomapLineList.end(), 0xff);
            std::fill(AutomapPolygonList.begin(), AutomapPolygonList.end(), 0xff);
            break;
    };
}
