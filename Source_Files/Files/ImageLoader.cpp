/*
 ImageLoader.cpp - Image file loading extras
 
 Written in 2001 by Christian Bauer (ImageDescriptor::LoadFromFile)
...
bool ImageDescriptor::LoadFromFile(const ao_path& path, int ImgMode, int flags, int actual_width, int actual_height, int maxSize)
{
    ...
    SDL_Surface* rgba = create_sdl_surface_32(Width, Height);
...
bool ImageDescriptor::LoadMipMapFromFile(DataFile& file, int flags, int level, DDSURFACEDESC2 &ddsd, int skip)
{
	...
	if (Format == RGBA8) 
	{
		...
		SDL_Surface* dst = SDL_CreateRGBSurfaceWithFormatFrom(buffer, dstWidth, dstHeight, 32, dstWidth * 4, AO_PIXEL_FORMAT_32);
...
void ImageDescriptor::PremultiplyAlpha()
{
	...
	for (int i = 0; i < GetNumPixels(); i++)
	{
		...
		if ((Pixels[i] & AO_ALPHA_MASK) == AO_ALPHA_MASK)
        {
            continue;
        }
		if ((Pixels[i] & AO_ALPHA_MASK) == 0)
        {
			Pixels[i] = 0;
			continue;
		}
