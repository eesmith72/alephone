...
static uint32_t calculate_file_crc(uint8_t* buffer, short buffer_size, DataFile& OFile);
...
uint32_t calculate_crc_for_file(const ao_path& path)
{
    uint32_t crc = 0;
    ...
    {
        crc = calculate_crc_for_opened_file(file);
    }
...
uint32 calculate_crc_for_opened_file(DataFile& file)
{
    ...
    if(build_crc_table())
    {
        ...
        crc= calculate_file_crc(buffer, BUFFER_SIZE, file);
        delete []buffer;
...
/* Calculate the crc for a file using the given buffer.. */
static uint32_t calculate_file_crc(uint8_t* buffer, int16_t buffer_size, DataFile& OFile)
{
    int64_t initial_position = OFile.get_position();
    int64_t file_length = OFile.get_length();
	OFile.set_position(0);

    uint32_t crc = 0xFFFFFFFFL;
	while (file_length)
	{
        int64_t count = file_length > buffer_size ? buffer_size : file_length;

        OFile.read(count, buffer);

		crc = calculate_buffer_crc(count, crc, buffer);
		file_length -= count;
	}
	
	OFile.set_position(initial_position);
