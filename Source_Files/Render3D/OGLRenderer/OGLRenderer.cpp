...
public:
	...
	Blur(GLuint w, GLuint h, Shader* s_blur, Shader* s_bloom) : _swapper(w, h, Bloom_sRGB), _shader_blur(s_blur),
                                                                _shader_bloom(s_bloom), _width(w), _height(h) {}
...
/*
 * override for Renderer::render_tree()
 *
...
void OGLRenderer::render_tree()
{
	...
	s->enable();
	s->setFloat(Shader::U_Time, view->effect_tick_count);
...
void OGLRenderer::clip_to_window(clipping_window_data *win)
{
...
void OGLRenderer::store_endpoint(
...
std::unique_ptr<TextureManager> OGLRenderer::setupSpriteTexture(const rectangle_definition& rect, short type, float offset, RenderStep renderStep) {
	...
	} else if (current_player->infravision_duration > 0) {
		color[0] = color[1] = color[2] = 1;
		convert_ogl_color_to_infravision(GET_COLLECTION_INDEX(GET_DESCRIPTOR_COLLECTION(rect.ShapeDesc)), color);
	...
	switch(transferMode) {
		...
		case _xfer_big_landscape:
			...
			if (current_player->infravision_duration > 0) {
				...
				convert_ogl_color_to_infravision(GET_COLLECTION_INDEX(GET_DESCRIPTOR_COLLECTION(Texture)), color);
				glColor4f(color[0], color[1], color[2], 1);
				if (opts->SphereMap)
				{
...
bool RenderModel(rectangle_definition& RenderRectangle, short Collection, short CLUT, float flare, float selfLuminosity, RenderStep renderStep)
{
    ...
    {
		color[0] = color[1] = color[2] = 1;
		convert_ogl_color_to_infravision(GET_COLLECTION_INDEX(GET_DESCRIPTOR_COLLECTION(RenderRectangle.ShapeDesc)), color);
		s = Shader::get(Shader::S_WallInfravision);
	}
