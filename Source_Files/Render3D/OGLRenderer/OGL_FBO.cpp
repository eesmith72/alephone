...
FBO::FBO(GLuint w, GLuint h, bool srgb) : _h(h), _w(w), _srgb(srgb)
{
	...
	assert_fail(glCheckFramebufferStatusEXT(GL_FRAMEBUFFER_EXT) == GL_FRAMEBUFFER_COMPLETE_EXT, "");
...
void FBOSwapper::activate()
{
    ...
    (draw_to_first ? first : second).activate(clear_on_activate);
...
void FBOSwapper::deactivate()
{
    ...
    (draw_to_first ? first : second).deactivate();
	active = false;
}
