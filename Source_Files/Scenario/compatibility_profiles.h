/*
 compatibility_profiles.h -- tell Aleph One exactly which bug fixes and
                            behaviors to use when starting a new game
                            or loading an older saved game/film file
...
#ifndef __compatibility_profile__
#define __compatibility_profile__

// was FilmProfile, but it affects more than just films; TODO: Classic M1-3 should use the appropriate profile to replicate original gameplay

// TODO: drop backwards compatibility with old saved game/film files (probably a good idea as they never worked right with solo or customizations, and it'll allow code to be cleaned up a bit; older files can still be used on older AO versions, ofc); keep the Classic M1-3 profiles and update if needed so they're historically accurate

#include "cstypes.hpp"


struct compatibility_profile_t
{
	// some LP bug fix
	bool keyframe_fix;
	...
	// Aleph One moved object initialization to improve Lua access; EES: now always early
...
extern compatibility_profile_t film_profile;


// TODO: why are there 2 enums and can they be merged into 1?
...
enum FilmProfileType
{
	...
	// if you add a film profile here, make sure to connect it in compatibility_profile_t.cpp
};
...
#endif /* __compatibility_profile__ */
