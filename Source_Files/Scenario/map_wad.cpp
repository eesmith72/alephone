...
void initialize_level_from_wad_data(wad_data* wad, bool is_saved_game, short version)
{
    ...
    unpack_line_data(data, count);
    AutomapLineList.resize(calculate_automap_list_size(count));
    ...
    unpack_polygon_data(data, count, version);
    AutomapPolygonList.resize(calculate_automap_list_size(count));
...
static uint8 *tag_to_global_array_and_size(uint32 tag, size_t *size)
{
	...
	switch (tag)
	{
		...
		case MAP_INDEXES_TAG:
			...
			break;
		case AUTOMAP_LINES:
            count= AutomapLineList.size();
			break;
		case AUTOMAP_POLYGONS:
            count= AutomapPolygonList.size();
			break;
	...
	*size= count*unit_size;
	...
	switch (tag)
	{
		...
		case MAP_INDEXES_TAG:
			...
			break;
		case AUTOMAP_LINES:
			memcpy(array, AutomapLineList.data(), *size);
			break;
		case AUTOMAP_POLYGONS:
			memcpy(array, AutomapPolygonList.data(), *size);
			break;
		case MONSTERS_STRUCTURE_TAG:
