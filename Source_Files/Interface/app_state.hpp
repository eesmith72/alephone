...
// TODO: may be simpler if advance_app_state_queuing_next is split into separate advance_app_state and set_next_app_state, and advance_app_state_now calls advance at start and gets back the new current state
...
{
    return get_app_state() >= app_state_t::startup_screen && get_app_state() <= app_state_t::shutdown_screen;
}



void advance_app_state_queuing_next(app_state_t new_state, uint32_t machine_ticks_until_next_state = 0); // 0 = transition now
...
void conclude_app_state_timeout(); // sets the timeout to 0 so app state will transition immediately on the next main event loop
