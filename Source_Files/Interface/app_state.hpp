...
// TODO: defined in game_event_loop, which is not ideal: problem occurs when exiting gameworld as app state is still `game_in_progress` but `main_screen.synchronize` needs to know that it's returning to UI in order to set correct size
bool game_loop_is_running(); // { return get_app_state() == app_state_t::game_in_progress; }
