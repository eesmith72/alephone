...
void Movie::StartExportingToFile(std::string path) {}
bool Movie::IsExporting() { return false; }
void Movie::StopExporting() {}
...
void FilmExporter::StartExportingToFile(std::string path)
{
    ...
    OpenALManager::Get()->ToggleDeviceMode(IsExporting());
    OpenALManager::Get()->Start();
}

bool FilmExporter::IsExporting()
{
  return (moviefile.length() > 0);
}

bool FilmExporter::Setup()
{
    if (!IsExporting())
        return false;
    if (!av)
...
uint64_t FilmExporter::GetCurrentAudioTimeStamp()
{
	return IsExporting() && av->inited ? current_audio_timestamp : 0;
}

int FilmExporter::Movie_EncodeThread(void *arg)
{
	reinterpret_cast<FilmExporter *>(arg)->EncodeThread();
	return 0;
}

void FilmExporter::EncodeVideo(bool last)
{
	if (av->yuv)
	{
...
void FilmExporter::EncodeAudio(bool last)
{
...
void FilmExporter::EncodeThread()
{
	av->video_counter = 0;
	av->prev_video_pts = 0;
	av->audio_counter = 0;
...
void FilmExporter::DequeueFrame(FrameQueue &queue, uint64_t tracknum, bool start_cluster)
{
...
void FilmExporter::DequeueFrames(bool last)
{
