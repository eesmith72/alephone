/*
 FilmExporter.h -- export a film recording to MPEG file
...
{
public:
	static FilmExporter *instance() { 
		static FilmExporter *m_instance = nullptr;
		if (!m_instance
				) m_instance = new FilmExporter(); 
		return m_instance; 
	}
	
	void StartExportingToFile(std::string path);
	bool IsExporting();
	void StopExporting();
