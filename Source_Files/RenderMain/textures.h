...
public:
    ...
    // important: row_count MUST be >0
	explicit bitmap_definition_buffer(int row_count) : buf(sizeof(bitmap_definition) + (row_count - 1) * sizeof(pixel8*)) {}
	
	bool empty() const { return buf.empty(); }
	int row_count() const { return empty() ? 0 : 1 + ((int32_t)buf.size() - sizeof(bitmap_definition)) / sizeof(pixel8*); }
